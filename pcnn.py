    ...
    def __init__(self,
        ...
        if word2vec is not None:
            ...
            if self.num_token == len(word2vec) + 2:
                unk = torch.randn(1, self.word_size) / math.sqrt(self.word_size)  # unknown token的embedding
        ...
        # Position Embedding
        self.pos1_embedding = nn.Embedding(2 * max_length, self.position_size, padding_idx=0)  # 初始化一个embedding，有2*max_length个词，每个词的长度是position_size
