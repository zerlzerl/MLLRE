...
def split_data(data, vocabulary, rel2cluster, task_num):  # -1 means all
    ...
    seperated_relations = [None] * task_num
    for rel, items in data.items():
        ...
        if seperated_relations[rel_cluster] is None:
            seperated_relations[rel_cluster] = [rel]
        else:
            seperated_relations[rel_cluster].append(rel)

    return separated_data, seperated_relations
